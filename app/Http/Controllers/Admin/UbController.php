...
class ExportController extends Controller
{
            ...
            foreach ($data as $val) {
                $excel->sheet($val->rank . '名-' . $val->name . '-' . $val->playerNumber,
                    function ($sheet) use ($val, $type) {
                        ...
                        $sheet->cell('A11', function ($cell) use ($val, $type) {
                            if ($type == 'certificate') {
                                $cell->setValue('獎　　　狀');
                            }
                            if ($type == 'completion') {
                        ...
                        $sheet->cell('A14', function ($cell) use ($val) {
                            $completeName = GameModel::where('gameSn', session('gameSn'))->value('completeName');
                        ...
                        $sheet->cell('H15', function ($cell) use ($val) {
                            $letter = GameModel::where('gameSn', session('gameSn'))->value('letter');
                        ...
                        $sheet->cell('F27', function ($cell) use ($val, $type) {
                            if ($type == 'certificate') {
                        ...
                        $sheet->cell('A49', function ($cell) use ($val) {
                            $date     = GameModel::where('gameSn', session('gameSn'))->value('date');
        ...
        \Excel::create($fileName, function ($excel) use ($teams, $fileName) {
            $excel->sheet($fileName, function ($sheet) use ($teams) {
                ...
                $sheet->cell('A1', function ($cell) {
                    $abridgeName = GameModel::where('gameSn', session('gameSn'))->value('abridgeName');
    ...
    {
        $schedule = ScheduleModel::where('gameSn', session('gameSn'))->get();
