                ...
                <div class="portlet-body">
                    @foreach($groups as $group)
                        <table class="table mb-0">
                            <tr>
                                <td>{{ $group->order }} </td>
                                <td class="text-center">{{ $group->game_type }} {{ $group->level }} {{ $group->group }} {{ $group->gender }}子組 {{ $group->item }}</td>
                                    ...
                                    <tr>
                                        <td class="w-20">
                                            @if (isset($group->players[$i]))
                                                {{ $group->players[$i]->player_number }} {{ $group->players[$i]->player->name }} {{ $group->players[$i]->player->agency }}
                                            @endif
                                        </td>
                                        <td class="w-20">
                                            @if (isset($group->players[$i + 1]))
                                                {{ $group->players[$i + 1]->player_number }} {{ $group->players[$i + 1]->player->name }} {{ $group->players[$i + 1]->player->agency }}
                                            @endif
                                        </td>
                                        <td class="w-20">
                                            @if (isset($group->players[$i + 2]))
                                                {{ $group->players[$i + 2]->player_number }} {{ $group->players[$i + 2]->player->name }} {{ $group->players[$i + 2]->player->agency }}
                                            @endif
                                        </td>
                                        <td class="w-20">
                                            @if (isset($group->players[$i + 3]))
                                                {{ $group->players[$i + 3]->player_number }} {{ $group->players[$i + 3]->player->name }} {{ $group->players[$i + 3]->player->agency }}
                                            @endif
                                        </td>
                                        <td class="w-20">
                                            @if (isset($group->players[$i + 4]))
                                                {{ $group->players[$i + 4]->player_number }} {{ $group->players[$i + 4]->player->name }} {{ $group->players[$i + 4]->player->agency }}
                                            @endif
                                        </td>
                                    </tr>
                                    @php
